...
export const sqlInjectionRule = {
  ...
  regex: /(?:(?:query|sql|execute|exec)\s*(?:=|\+=)\s*(?:['"`].*?\+.*?['"`]|`.*?\$\{.*?\}.*?`)|\.(?:query|execute|raw)\s*\(\s*(?:['"`].*?\$\{.*?\}.*?['"`]|.*?\+.*?\))|(?:cursor\.execute|execute)\s*\(\s*(?:['"].*?%s.*?['"]\s*%|f['"].*?\{.*?\}.*?['"]))/gi,
 ...
 */
export class SqlInjectionCodeActionProvider implements vscode.CodeActionProvider {
  ...
  provideCodeActions(
    ...
    if (document.languageId === 'python') {
      hintText += '// Example: cursor.execute("SELECT * FROM users WHERE id = ?", (user_id,))\n';
    } else {
      hintText += '// Example: db.query("SELECT * FROM users WHERE id = ?", [userId])\n';
    }
    ...
    learnMoreAction.command = {
      ...
      arguments: [sqlInjectionRule.relatedInfo?.url, sqlInjectionRule.description]
    };
